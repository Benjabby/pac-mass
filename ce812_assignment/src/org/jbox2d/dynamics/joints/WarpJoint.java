...
public class WarpJoint extends Joint 
{
	// KINDA KLUDGE
	...
	protected WarpJoint(IWorldPool argWorldPool, WarpJointDef def)
	{
	    super(argWorldPool, def);
	    
	    m_groundAnchorA.set(def.mazeLocalGroundAnchorA);
	    m_groundAnchorB.set(def.mazeLocalGroundAnchorB);
	    ...
	    m_lengthA = m_bodyA.getWorldPoint(m_localAnchorA).sub(maze.getBody().getWorldPoint(m_groundAnchorA)).length();
	    m_lengthB = m_bodyB.getWorldPoint(m_localAnchorB).sub(maze.getBody().getWorldPoint(m_groundAnchorB)).length();
//	    m_constant = 0.0f;
	    m_constant = (m_ratio==0?0:m_lengthA + m_ratio * m_lengthB);
	    m_impulse = 0.0f;
	  }
	...
	public float getCurrentLengthA() {
		...
		p.subLocal(maze.getBody().getWorldPoint(m_groundAnchorA));
	...
	public float getCurrentLengthB() {
		...
		p.subLocal(maze.getBody().getWorldPoint(m_groundAnchorB));
		float length = p.length();
		pool.pushVec2(1);
		return length;
	}
	...
	public Vec2 getGroundAnchorA() {
		return maze.getBody().getWorldPoint(m_groundAnchorA);
	}
	
	// KINDA KLUDGE
	public Vec2 getGroundAnchorB() {
		return maze.getBody().getWorldPoint(m_groundAnchorB);
	}
	...
	public void initVelocityConstraints(final SolverData data) {
		...
		m_uA.set(cA).addLocal(m_rA).subLocal(maze.getBody().getWorldPoint(m_groundAnchorA));
		m_uB.set(cB).addLocal(m_rB).subLocal(maze.getBody().getWorldPoint(m_groundAnchorB));
...
//	    data.velocities[m_indexB].v.set(vB);
	...
	public boolean solvePositionConstraints(final SolverData data) {
		...
		uA.set(cA).addLocal(rA).subLocal(maze.getBody().getWorldPoint(m_groundAnchorA));
		uB.set(cB).addLocal(rB).subLocal(maze.getBody().getWorldPoint(m_groundAnchorB));
